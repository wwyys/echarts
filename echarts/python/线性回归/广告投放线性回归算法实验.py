...
#清洗不需要的数据
#adv_data.values.reshape(-1, 1)
new_adv_data=adv_data.iloc[:,1:]
#得到我们所需要的数据集且查看其前几列以及数据形状
print(new_adv_data.head())
#new_adv_data.drop（['Unnamed:0'],axis=1) “未命名：0”列是多余的。所以，我们把这列删除
...
基于数据，可以得出线性近似。对数据集给出一条拟合直线并查看等式参数就是这么简单。
...
'''
建模
和一元线性回归的建模过程一致，定义特征值和目标变量，并利用scikit_learn库来执行线性回归模型
得到如下等式：
The linear model is :Y=2.9389+0.045765*TV+0.18853*Radio+-0.0010375*Newspaper
'''
Xs=new_adv_data.drop(['Sales'],axis=1)  #‘Unnamed’去除Ales列
y=new_adv_data['Sales'].values.reshape(-1,1)
reg=LinearRegression()
reg.fit(Xs,y)
print("The linear model is :Y={:.5}+{:.5}*TV+{:.5}*Redio+{:.5}*Newspaper".format(reg.intercept_[0],reg.coef_[0][0],reg.coef_[0][1],reg.coef_[0][2]))
X=np.column_stack((new_adv_data['TV'],new_adv_data['Redio'],new_adv_data['Neswpaper']))
y=new_adv_data['Sales']
X2=sm.add_constant(X)
est=sm.OLS(y,X2)
est2=est.fit()
...
#print("new_adv_data.ix[:,:3]",new_adv_data.ix[:,:3])
#print("new_adv_data.Sales",new_adv_data.Sales)
...
plt.figure()
plt.plot(range(len(Y_pred)),Y_pred,'b',label="predict")
plt.plot(range(len(Y_pred)),Y_pred,'r',label="test")
